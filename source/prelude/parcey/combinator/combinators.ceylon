...
import prelude.parcey.internal {
    chooseName,
    simplePlural,
    computeParserName
}
...
"Creates a Parser that applies each of the given parsers in sequence.
 ...
 If any of the parsers fails, the chain is broken and a [[prelude.parcey::ParseError]]
...
"Creates a Parser which attempts to parse input using the first of the given parsers, and in case it fails,
 ...
 If any Parser fails, the [[prelude.parcey::CharacterConsumer.consumed]] stream from its result is chained to the actual input
